...
def substrings(string)
    ...
    (0...string.length).each do |i|
        (i...string.length).each do |j|
            ...
            substrings << new_string if !substrings.include?(new_string)
        end
    end
...
p subwords("erbearsweatmyajs", dictionary) # ["bears", "ear", "a"]
...
def concatenate(strings)
    strings.inject("") { |conc, el| conc += el }
end
